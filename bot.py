...
# --- Асинхронний клас для безпечного зберігання даних (Критично для Webhook) ---
class ComboStorage:
    ...
    def __init__(self):
        ...
        self._source_url = ""
        # Використовуємо Lock для запобігання пошкодженню даних при одночасному записі
...
async def start_handler(m: types.Message):
    ...
    # Використовуємо Markdown для @CryptoComboDaily, щоб не було помилок парсингу
    await m.answer(
...
async def show_combo(c: types.CallbackQuery):
    ...
    # Використовуємо HTML для форматування жирним
    await c.message.edit_text(
        f"<b>Комбо на {datetime.now():%d.%m.%Y}</b>\n\n{combo_text_data}", 
        parse_mode="HTML",
        # Додаємо клавіатуру "Назад"
...
async def set_webhook_and_clear_updates():
    ...
    try:
        # Видаляємо Webhook, щоб очистити всі зависші оновлення перед встановленням нового
