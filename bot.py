...
import json
import httpx
import re # Додано для більш розумного екранування
...
# ЗМІНЕНО: Використання звичайного рядка, щоб уникнути конфліктів синтаксису при завантаженні
    ...
    if os.path.exists(DB_FILE):
        try:
            with open(DB_FILE, 'r', encoding='utf-8') as f:
                ...
                # ВИПРАВЛЕНО: Зберігаємо вміст як звичайний рядок
...
# Символи, які потрібно екранувати для Markdown V2
MARKDOWN_V2_SPECIAL_CHARS = r"([\[\]()~>#+=|{}.!-])"
# УВАГА: * і ` (жирний шрифт, inline code) не екрануємо тут.
...
def escape_all_except_formatting(text: str) -> str:
    ...
    # 1. Екрануємо символи, крім тих, що використовуються для форматування
    # _ та * екрануються, лише якщо вони НЕ оточені іншими символами форматування.
    
    # Регулярний вираз для екранування всіх спецсимволів, окрім тих, що можуть бути частиною 
    # жирного шрифту (**) або inline коду (`)
    
    # Екрануємо зворотний слеш (його завжди потрібно екранувати в першу чергу)
    text = text.replace('\\', r'\\')
    
    # Екрануємо решту критичних символів ([, ], (, ), ~, >, #, +, -, =, |, {, }, ., !)
    text = re.sub(MARKDOWN_V2_SPECIAL_CHARS, r'\\\1', text)
    
    # Екрануємо _ та * тільки якщо вони не оточені відповідними символами форматування
    text = text.replace('*', r'\*')
    text = text.replace('_', r'\_')
    text = text.replace('`', r'\`') # Екрануємо `
    
    # Намагаємося зберегти тільки **...** та `...`
    # Складність: автоматичне збереження ** та ` без повної рекурсивної перевірки 
    # є надзвичайно складною. Найбезпечніше - екранувати все, а потім 
    # явно "де-екранувати" потрібні комбінації.

    # 1. Замінюємо тимчасовим placeholder'ом жирний шрифт (**текст**)
    def replace_bold(match):
        # Якщо всередині жирного шрифту є екрановані символи, прибираємо екранування
        # Тільки для жирного шрифту та inline-коду
    ...
    # 3. Виконуємо загальне екранування
    # Знову екрануємо всі спеціальні символи, які можуть бути поза форматуванням
    
    # 4. Повертаємо жирний шрифт та inline-код на місце (БЕЗ екранування їх самих)
    ...
    # Додатково екрануємо **\_** (жирний курсив) та інші комбінації
    # Оскільки aiogram3 має покращений парсер, спробуємо мінімальне екранування.
    # Залишаємо екранування для _ та * тільки для тих, що залишилися.
...
def _build_start_message_content(user_name: str, user_id: int, is_admin: bool):
    ...
    # Імена не повинні містити спецсимволів, але для надійності екрануємо
    escaped_user_name = escape_all_except_formatting(user_name)
    
    # Тут використовуємо сирий рядок (r"...") для уникнення помилок при екрануванні
...
""".format(escaped_user_name=escaped_user_name, status_text=status_text)
    ...
    # Екрануємо весь текст, крім потрібного форматування
...
async def command_combo_handler(message: types.Message, bot: Bot) -> None:
    ...
    if is_admin or IS_ACTIVE or is_premium:
        ...
        # 3. Створюємо фінальний текст, замінюючи placeholder дати, якщо він є у COMBO_CONTENT
        if "{date_str}" in COMBO_CONTENT:
            combo_text_with_date = combo_content_to_send.format(date_str=date_str_escaped)
        else:
            # Якщо адмін не додав placeholder, додаємо дату на початок і екрануємо заголовок
...
async def inline_callback_handler(callback: types.CallbackQuery, bot: Bot):
    ...
    if user_id == ADMIN_ID:
        ...
        if callback.data == "back_to_start":
            ...
            # ВИПРАВЛЕНО: використовуємо edit_text
...
"""
                ...
                # Екрануємо статус перед відображенням
    ...
    for attempt in range(3):
        try:
            ...
            if attempt < 2:
                ...
                # Для виробничого коду краще викликати виняток або повернути порожній словник
                return {} # Повертаємо порожній словник у разі невдачі
