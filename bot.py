...
# Якщо змінних немає, видаємо конкретну помилку для діагностики
if not BOT_TOKEN:
    logger.error("КРИТИЧНА ПОМИЛКА: Змінна BOT_TOKEN не встановлена.")
    raise RuntimeError("❌ BOT_TOKEN не встановлено")
if not WEBHOOK_HOST:
    logger.error("КРИТИЧНА ПОМИЛКА: Змінна WEBHOOK_HOST не встановлена.")
    raise RuntimeError("❌ WEBHOOK_HOST не встановлено. Перевірте змінні середовища Railway.")
...
async def start_handler(m: types.Message):
    ...
    Додано логування, щоб бачити, коли команда дійсно приходить.
    """
    logger.info(f"ХЕНДЛЕР: Отримано /start від user={m.from_user.id}")
...
@dp.message()
async def fallback_handler(m: types.Message):
    """
    Обробляє будь-яке текстове повідомлення, яке не є командою чи хендлером.
    Це "оживляє" бота, якщо користувач просто щось напише.
    """
    logger.info(f"ХЕНДЛЕР: Отримано fallback-повідомлення від user={m.from_user.id}. Повертаю головне меню.")
    await m.answer(
        "Я розумію лише команди або кнопки. Повертаю головне меню:",
        reply_markup=get_main_menu_keyboard(m.from_user.id)
    )
