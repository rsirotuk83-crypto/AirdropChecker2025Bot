...
async def process_get_combo(callback_query: types.CallbackQuery, bot: Bot):
    ...
    if is_admin:
         # Використовуємо _scrape_for_combo, якщо функція main_scheduler не працює
...
async def process_force_scrape(callback_query: types.CallbackQuery, bot: Bot):
    ...
    try:
        # Виклик синхронної функції скрапінгу в окремому потоці, щоб не блокувати aiogram
        # Вона оновить GLOBAL_COMBO_CARDS
